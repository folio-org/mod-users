...
public class UserTenantRepository {
  ...
  private static final String INSERT_SQL = "INSERT INTO %s.%s (id, user_id, username, tenant_id, creation_date, central_tenant_id, email, mobile_phone_number, phone_number, barcode, external_system_id)" +
  ...
  public Future<Boolean> saveUserTenant(Conn conn, UserTenant userTenant, String tenantId) {
    ...
    Tuple queryParams = Tuple.of(userTenant.getId(), userTenant.getUserId(), userTenant.getUsername(),
      ...
      userTenant.getEmail(), userTenant.getMobilePhoneNumber(), userTenant.getPhoneNumber(), userTenant.getBarcode(), userTenant.getExternalSystemId());
