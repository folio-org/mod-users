...
class ExpirationToolTest {
  ...
  @Test
  void pgConnectionClosesOnUpdateDBException(Vertx vertx) {
    final var postgresClient = mock(PostgresClient.class);
    final var expirationTool = new ExpirationTool((v, t) -> postgresClient);

    when(postgresClient.closeClient())
      .thenReturn(Future.failedFuture("Mocked Failure"));

    when(postgresClient.update(anyString(), any(User.class), any()))
      .thenThrow(new RuntimeException("Mocked Exception"));

    expirationTool.disableUserActuallyCloseConnectionExample(vertx, "myTenant", new User());

    verify(postgresClient, atLeastOnce()).closeClient();
  }

  @Test
  void pgConnectionClosesOnUpdateDBExceptionExampleShowingNoClosureInOriginalCode(Vertx vertx) {
    final var postgresClient = mock(PostgresClient.class);
    final var expirationTool = new ExpirationTool((v, t) -> postgresClient);
    ...
    // The original one doesn't close connection.
    expirationTool.disableUser(vertx, "myTenant", new User());

    // This should not pass because closeClient() needs to be called.
    verify(postgresClient, never()).closeClient();
  }
}
